...
class traf::buildtest {
  ...
  if $::osfamily == 'RedHat' {
    ...
    exec { 'rsync-build-tool-tgz' : 
        ...
        user      => jenkins, 
        provider  => shell, 
        onlyif    => "/usr/bin/test `/usr/bin/rsync -haS --dry-run --itemize-changes --del -e \"ssh -o StrictHostKeyChecking=no\" jenkins@downloads.trafodion.org:/srv/static/downloads/build-tool-tgz /opt/traf | wc -l` -gt 0", 

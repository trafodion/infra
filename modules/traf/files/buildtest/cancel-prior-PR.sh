...
# ### TEMP
# ##### -k option is temporary for testing on server with self-signed cert
# jenkins server data cmd - jenkins provides JENKINS_URL
API="curl -s -k $JENKINS_URL/job/$PROJ_NAME"
...
### TEMP  -k option
curl -X POST -k -s -u ${USER}:$PW $JENKINS_URL/job/$PROJ_NAME/$MyBuild/stop

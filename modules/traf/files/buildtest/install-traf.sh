...
trafball="$(/bin/ls $(pwd)/trafodion/core/trafodion_server-*.tgz)"
dcsball="$(/bin/ls $(pwd)/trafodion/dcs/target/dcs*gz)"
instball="$(/bin/ls $(pwd)/trafodion/install/installer*gz)"

flist="$instball $trafball $dcsball"
if [[ $REGRESS == "regress" ]]
then
  regball="$(/bin/ls $(pwd)/trafodion/core/trafodion-regress.tgz)"
  flist+=" $regball"
fi

...
# NOTE: tests should be delimited by a comma with no space.  i.e. TestBasic,SomeOtherTestName
if [ $# -gt 4 ]; then
  shift
  shift
  shift
...
if [ -z "$TESTS" ]; then
  ./jdbc_test.py --appid=jdbc_test --target=localhost:37800 --user=dontcare --pw=dontcare --javahome=$JAVA_HOME --jdbctype=T4 --jdbccp=$WORKSPACE/$TRAF_DIR/sqf/export/lib/jdbcT4.jar
  jdbcRes=$?
  ...
  ./jdbc_test.py --appid=jdbc_test --target=localhost:37800 --user=dontcare --pw=dontcare --javahome=$JAVA_HOME --jdbctype=T4 --jdbccp=$WORKSPACE/$TRAF_DIR/sqf/export/lib/jdbcT4.jar --tests=$TESTS
  jdbcRes=$?
fi

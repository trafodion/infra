...
for PROJECT in $(cat projects.txt); do
    ...
    change_id=""
    # See if there is an open change in the trafodion/requirements topic
    # If so, get the change id for the existing change for use in the
    # commit msg.
    change_info=$(ssh -p 29418 review.trafodion.org gerrit query --current-patch-set status:open project:$PROJECT topic:$TOPIC owner:$USERNAME branch:$BRANCH)
...
EOF
    ...
    rm -rf $PROJECT_DIR
    git clone ssh://$USERNAME@review.trafodion.org:29418/$PROJECT.git
    pushd $PROJECT_DIR

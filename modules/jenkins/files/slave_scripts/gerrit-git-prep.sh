...
if [ -z "$GERRIT_SITE" ]
then
  echo "The gerrit site name (eg 'https://review.trafodion.org') must be the first argument."
  exit 1
fi

if [ -z "$ZUUL_SITE" ]
then
  echo "The zuul site name (eg 'http://zuul.trafodion.org') must be the second argument."
  exit 1
fi
...
then
    ...
    # git://git.trafodion.org/
    # https://review.trafodion.org/p
fi

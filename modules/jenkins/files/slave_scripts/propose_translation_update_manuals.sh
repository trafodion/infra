...
PROJECT="trafodion-manuals"
DocFolder="doc"

COMMIT_MSG="Imported Translations from Transifex"

git config user.name "Trafodion Jenkins"
git config user.email "jenkins@trafodion.org"
git config gitreview.username "Zuul"
...
change_info=`ssh -p 29418 review.trafodion.org gerrit query --current-patch-set status:open project:trafodion/$PROJECT topic:transifex/translations owner:jenkins`
...
# because there is an existing .tx folder in trafodion-manuals
...
> ${DocFolder}/high-availability-guide/bk-ha-guide.xml
    ...
    then 
        ...
        # Set auto-local
        tx set --auto-local -r trafodion-manuals-i18n.${DOCNAME} \

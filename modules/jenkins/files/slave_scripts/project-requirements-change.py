...
# Copyright (c) 2013 Hewlett-Packard Development Company, L.P.Hewlett-Packard Development Company, L.P.Hewlett-Packard Development Company, L.P.Hewlett-Packard Development Company, L.P.
#
...
class RequirementsList(object):
    ...
    def read_requirements(self, fn, ignore_dups=False):
        ...
        for line in open(fn):
            ...
            line = line.strip()
            if (not line or
                line.startswith('http://downloads.trafodion.org/') or
...
def main():
    ...
    run_command("git clone https://review.trafodion.org/p/trafodion/"
                "requirements --depth 1 %s" % reqdir)
    os.chdir(reqdir)
    run_command("git checkout remotes/origin/%s" % branch)
    print "requirements git sha: %s" % run_command(
        "git rev-parse HEAD").strip()
    os_reqs = RequirementsList('trafodion/requirements')
    ...
    for req in head_reqs.reqs.values():
        ...
        if name not in os_reqs.reqs:
            print("Requirement %s not in trafodion/requirements" % str(req))
            failed = True
            continue
        ...
        if not (req == os_reqs.reqs[name]):
            print("Requirement %s does not match trafodion/requirements "
    ...
    if failed or os_reqs.failed or head_reqs.failed or branch_reqs.failed:
        sys.exit(1)
    print("Updated requirements match trafodion/requirements.")
